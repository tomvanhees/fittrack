...
describe('<SetRow />', () => {
  it('toont de waarden van de vorige sessie als referentie', () => {
    render(
      <SetRow setNumber={1} previousSet={previousSet} editable onSave={jest.fn()} />
    );
    expect(screen.getByText('S1')).toBeOnTheScreen();
    expect(screen.getByText('80kg×8')).toBeOnTheScreen();
  });

  it('toont een streepje wanneer er geen vorige set is', () => {
    render(<SetRow setNumber={1} editable onSave={jest.fn()} />);
